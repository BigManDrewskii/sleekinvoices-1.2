...
export function EmailAnalyticsWidget({ className }: EmailAnalyticsWidgetProps) {
  ...
  const chartData = dataPoints.map(d => ({
    ...
    label:
      period === "monthly"
        ? new Date(d.period + "-01").toLocaleDateString("en-US", {
            month: "short",
          })
        : period === "weekly"
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div
          className="bg-card border border-border rounded-lg px-4 py-3 shadow-xl"
          role="tooltip"
          aria-label={`Data for ${label}: Open rate ${payload[0]?.value || 0}%, Click rate ${payload[1]?.value || 0}%`}
        >
    ...
    if (value > 0) {
      return (
        <div
          ...
          role="status"
          aria-label={`Trending up ${value.toFixed(1)}%`}
        >
      ...
      return (
        <div
          ...
          role="status"
          aria-label={`Trending down ${Math.abs(value).toFixed(1)}%`}
        >
    ...
    <Card className={className}>
      <CardContent className="p-6">
        ...
        <div
          className="h-[200px]"
          role="img"
          aria-label="Email performance chart showing open and click rates over time"
        >
          {hasData ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Send invoices to see email performance
              </p>
            </div>
          )}
        </div>
