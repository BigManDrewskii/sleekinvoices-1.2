  ...
  const currentAvatar = () => {
    if (selectedType === "upload" && uploadedFile) {
      return (
        <img
          src={uploadedFile}
          alt={`${userName}'s uploaded avatar`}
    ...
    if (selectedType === "boring") {
      ...
      const [style, seed] = (
        currentAvatarUrl || `${selectedStyle}:${userEmail || userName}`
      ).split(":");
      return (
        <Avatar
          size={96}
      ...
      <div className="flex-1 space-y-4">
        ...
        <div className="text-xs text-muted-foreground space-y-1">
          {selectedType === "initials" && (
            <p>Using your initials: {getInitials()}</p>
          )}
          {selectedType === "boring" && (
            <p>
              Generated avatar - Click "Generate Avatar" again for a different
              style
            </p>
          )}
          {selectedType === "upload" && (
            <p>Custom photo (PNG, JPG, WEBP up to 5MB)</p>
          )}
        </div>
      </div>
    </div>
