...
export function PortalAccessDialog({
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.clientPortal.getActiveAccess.useQuery(
    { clientId: client.id },
    { enabled: open }
  );
  ...
  const portalUrl = activeAccess
    ? `${window.location.origin}/portal/${activeAccess.accessToken}`
    : "";
  const isExpired = activeAccess
    ? new Date() > new Date(activeAccess.expiresAt)
    : false;
      ...
      <DialogContent className="sm:max-w-[550px]">
        <DialogHeader className="pb-4">
          <DialogTitle className="flex items-center gap-3">
            ...
            <span className="text-xl">Client Portal Access</span>
          </DialogTitle>
            ...
            <>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {formatDate(activeAccess.createdAt)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Expires:</span>
                    <span className="font-medium">
                      {formatDate(activeAccess.expiresAt)}
                    </span>
                  </div>
                  {activeAccess.lastAccessedAt && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatDate(activeAccess.lastAccessedAt)}
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Share this link with your client to give them access to view
              ...
              {!client.email && (
                <p className="text-xs text-yellow-600 bg-yellow-500/10 border border-yellow-500/20 p-3 rounded-lg">
                  ⚠️ Client email is required to send invitations. Please add an
                  email address to this client.
                </p>
              )}
            </>
          ) : (
              ...
              <div className="rounded-lg border border-dashed border-border bg-secondary/30 p-8 text-center space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Generate a secure link for {client.name} to access their
                    invoices
                  </p>
                </div>
                <Button
        ...
        <DialogActions onClose={() => onOpenChange(false)} cancelText="Close" />
      </DialogContent>
    </Dialog>
  );
}
