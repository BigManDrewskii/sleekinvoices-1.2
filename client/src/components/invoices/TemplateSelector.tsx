...
export function TemplateSelector({ value, onChange, error }: TemplateSelectorProps) {
  ...
  const selectedTemplate = value ? templates?.find(t => t.id === value) : defaultTemplate;
  ...
  return (
    <div className="space-y-2">
      ...
      <Select
        ...
        onValueChange={(val) => onChange(val === "default" ? null : parseInt(val))}
      >
            ...
            {selectedTemplate ? (
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{ backgroundColor: selectedTemplate.primaryColor || "#5f6fff" }}
                />
        ...
        <SelectContent>
          ...
          <SelectItem value="default">
            <div className="flex items-center gap-2">
              <div
                className="w-4 h-4 rounded-sm"
                style={{ backgroundColor: defaultTemplate?.primaryColor || "#5f6fff" }}
              />
          ...
          {templates?.map((template) => (
            <SelectItem key={template.id} value={template.id.toString()}>
              <div className="flex items-center gap-2">
                <div
                  className="w-4 h-4 rounded-sm"
                  style={{ backgroundColor: template.primaryColor || "#5f6fff" }}
                />
      ...
      <p className="text-xs text-muted-foreground">
        Choose a template for this invoice. Leave as default to use your default template.
      </p>
    </div>
  );
}
