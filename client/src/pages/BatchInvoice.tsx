...
} from "@/components/BatchTemplateDialog";
...
export default function BatchInvoice() {
  ...
  const [selectedClientIds, setSelectedClientIds] =
    useState<number[]>(initialClientIds);
  ...
  // Usage tracking state (for template loading)
  ...
  const { data: batchTemplates, refetch: refetchBatchTemplates } =
    trpc.batchTemplates.list.useQuery(undefined, {
      enabled: isAuthenticated,
    });

  const { data: clientsByTag, isLoading: isLoadingClientsByTag } =
    trpc.clients.getClientsByTag.useQuery(
      { tagId: selectedTagId! },
  ...
  const incrementTemplateUsage =
    trpc.batchTemplates.incrementUsage.useMutation();
  ...
  const handleAddClientsFromTag = () => {
    ...
    const uniqueIds = Array.from(
      new Set([...selectedClientIds, ...newClientIds])
    );
    ...
    if (addedCount > 0) {
      toast.success(
        `Added ${addedCount} client${addedCount !== 1 ? "s" : ""} from tag`
      );
    } else {
  ...
  const handleLoadTemplate = async (batchTemplateId: number) => {
    try {
      ...
      const result = await utils.batchTemplates.get.fetch({
        id: batchTemplateId,
      });
  ...
  const handleCreateBatch = async () => {
    ...
    if (lineItems.every(item => !item.description || item.rate === 0)) {
      toast.error(
        "Please add at least one line item with description and rate"
      );
      return;
    }
    ...
    for (let i = 0; i < selectedClients.length; i++) {
      ...
      try {
        ...
        if (
          errorMessage.includes("Monthly invoice limit reached") ||
          errorMessage.includes("invoice limit")
        ) {
          setShowUpgradeDialog(true);
          // Mark remaining as error
          for (let j = i; j < selectedClients.length; j++) {
            results[j] = {
              ...results[j],
              status: "error",
              error: "Invoice limit reached. Please upgrade to Pro.",
            };
          }
          setBatchResults([...results]);
          break;
    ...
    if (successCount === selectedClients.length) {
      ...
      toast.warning(
        `Created ${successCount} of ${selectedClients.length} invoices`
      );
    }
  };
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-muted-foreground mb-4">
                ...
                or use tags to quickly select groups of clients.
              </p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Create invoices for {selectedClients.length} selected client
          ...
          <div className="space-y-6">
            ...
            <Card>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {selectedTagId && (
                        <Button
                          ...
                          disabled={
                            isLoadingClientsByTag || !clientsByTag?.length
                          }
                        >
                          {isLoadingClientsByTag ? (
                            ...
                            <>
                              Add {clientsByTag?.length || 0} Client
                              {(clientsByTag?.length || 0) !== 1 ? "s" : ""}
                            </>
                          )}
                        </Button>
                      )}
              ...
              <CardContent>
                ...
                {selectedClients.length === 0 && (
                  <p className="text-sm text-muted-foreground">
                    No clients selected. Use the tag filter above to add
                    clients.
                  </p>
                )}
              </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  These items will be added to all {selectedClients.length}{" "}
                  invoices
                </CardDescription>
              </CardHeader>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={status}
                    onValueChange={(v: "draft" | "sent") => setStatus(v)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              >
                {isCreating ? (
                  ...
                  <>
                    ...
                    Create {selectedClients.length} Invoice
                    {selectedClients.length !== 1 ? "s" : ""}
                  </>
                )}
              </Button>
