...
export default function SubscriptionHistory() {
  ...
  // Calculate totals
  const completedPayments = items.filter(
    item => ['finished', 'confirmed', 'active'].includes(item.status.toLowerCase())
  );
  const totalSpent = completedPayments.reduce((sum, item) => sum + item.amount, 0);
  const totalMonths = completedPayments.reduce((sum, item) => sum + item.months, 0);
            ...
            <CardHeader>
              ...
              <CardDescription>
                {totalCryptoPayments > 0 
                  ...
                  : 'No payments yet'}
                {history?.hasStripeSubscription && ' • Stripe subscription active'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {items.length === 0 ? (
                          ...
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <span className="font-medium">
                                {item.type === 'crypto' ? 'Crypto Payment' : 'Stripe Subscription'}
                              </span>
                              {item.isExtension && (
                                <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-500/20 text-purple-500">
                                  Extension
                                </span>
                              )}
                            ...
                            <p className="text-sm text-muted-foreground">
                              {getDurationLabel(item.months)} • {formatDate(item.date)}
                            </p>
            ...
            <CardContent className="p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-muted-foreground">
                  <p className="font-medium text-foreground mb-1">About Payment History</p>
                  <p>
                    This page shows all your subscription payments including one-time crypto payments 
                    and recurring Stripe subscriptions. Crypto payments are processed via NOWPayments 
                    and may take a few minutes to confirm on the blockchain.
                  </p>
                </div>
              </div>
            </CardContent>
