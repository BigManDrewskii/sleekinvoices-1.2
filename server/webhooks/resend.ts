...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    if (!event.type || !event.data?.email_id) {
      console.warn('[Resend Webhook] Invalid event payload:', JSON.stringify(event));
    ...
    console.log(`[Resend Webhook] Received ${event.type} for message ${messageId}`);
    ...
    if (!emailLog) {
      ...
      console.log(`[Resend Webhook] No email log found for messageId: ${messageId}`);
    ...
    switch (event.type) {
      ...
      case 'email.bounced':
        ...
        console.log(`[Resend Webhook] Email bounced for ${event.data.to[0]}: ${event.data.bounce?.message}`);
        break;
