...
describe('GlobalSearch - Refined Modal UI', () => {
  it('should have correct modal structure', () => {
    const modalConfig = {
      ...
      container: 'fixed inset-x-0 top-0 z-50 flex items-start justify-center pt-[10vh] sm:pt-[15vh] px-4',
      modal: 'w-full max-w-lg bg-card border border-border rounded-xl shadow-2xl overflow-hidden',
    };
  ...
  it('should have result item styling with icons', () => {
    const resultItemConfig = {
      invoiceIcon: {
        container: 'flex h-9 w-9 items-center justify-center rounded-lg bg-primary/10 text-primary',
        icon: 'h-4 w-4',
      },
      clientIcon: {
        container: 'flex h-9 w-9 items-center justify-center rounded-lg bg-green-500/10 text-green-500',
        icon: 'h-4 w-4',
      },
      arrowIcon: 'h-4 w-4 text-muted-foreground/50 opacity-0 group-hover:opacity-100',
    };
...
describe('Navigation - Search Button', () => {
  ...
  it('should show keyboard shortcut on md+ viewports', () => {
    const kbdConfig = {
      className: 'hidden md:inline-flex pointer-events-none h-5 select-none items-center gap-1 rounded border border-border/50 bg-muted/50 px-1.5 font-mono text-[10px] font-medium text-muted-foreground',
      content: '⌘K',
    };
