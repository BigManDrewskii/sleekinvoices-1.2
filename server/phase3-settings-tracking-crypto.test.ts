...
import { users, invoices, clients, invoiceViews, payments } from '../drizzle/schema';
...
describe('Phase 3: Settings, View Tracking, and Crypto Payments', () => {
  ...
  afterAll(async () => {
    if (db && testUserId) {
      ...
      await db.delete(invoiceViews).where(eq(invoiceViews.invoiceId, testInvoiceId));
  ...
  describe('3.3 Crypto Payment Support', () => {
    ...
    it('should store high-precision crypto amounts', async () => {
      ...
      expect(parseFloat(result.cryptoAmount!)).toBeCloseTo(0.123456789012345678, 10);
    });
