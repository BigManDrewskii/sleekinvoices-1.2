...
describe('Client Portal Crypto Payment', () => {
  ...
  describe('createCryptoPayment validation', () => {
    ...
    it('should calculate remaining amount correctly', async () => {
      ...
      const invoice = await db.getInvoiceById(1, 100);
      const remaining = parseFloat(invoice!.total) - parseFloat(invoice!.amountPaid || '0');
  ...
  describe('NOWPayments invoice creation', () => {
    it('should create invoice with correct parameters', async () => {
      ...
      vi.mocked(nowpayments.createInvoice).mockResolvedValue(mockPayment as any);
      ...
      const result = await nowpayments.createInvoice({
        ...
        successUrl: 'https://example.com/portal/token?payment=success&invoiceId=1',
        cancelUrl: 'https://example.com/portal/token?payment=cancelled&invoiceId=1',
        isFixedRate: true,
      });
    ...
    it('should not specify payCurrency to allow customer choice', async () => {
      vi.mocked(nowpayments.createInvoice).mockImplementation(async (params: any) => {
        // Verify payCurrency is not specified
        expect(params.payCurrency).toBeUndefined();
        return { id: 'payment-123', invoice_url: 'https://nowpayments.io/payment/?iid=123' } as any;
      });
