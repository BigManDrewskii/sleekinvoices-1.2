...
export const appRouter = router({
    ...
    exportAllData: protectedProcedure
      ...
      if (format === 'csv') {
        ...
        if (allLineItems.length > 0) {
          archive.append(arrayToCSV(allLineItems), { name: 'invoice_line_items.csv' });
        }
        ...
        if (recurringInvoices.length > 0) {
          archive.append(arrayToCSV(recurringInvoices), { name: 'recurring_invoices.csv' });
        }
  ...
  emailHistory: router({
    ...
    analyticsOverTime: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        const { logs } = await db.getEmailLogsByUserId(ctx.user.id, { limit: 10000 });
        ...
        for (const log of filteredLogs) {
          ...
          if (periodData[periodKey]) {
            ...
            if (log.deliveryStatus === 'delivered' || log.deliveryStatus === 'opened' || log.deliveryStatus === 'clicked') {
        ...
        const dataPoints = Object.values(periodData).sort((a, b) => a.period.localeCompare(b.period));
        ...
        const totalClicked = filteredLogs.filter(l => l.deliveryStatus === 'clicked').length;
        
        // Calculate previous period for comparison
        const prevStartDate = new Date(startDate.getTime() - input.days * 24 * 60 * 60 * 1000);
        ...
        const currentOpenRate = totalDelivered > 0 ? (totalOpened / totalDelivered) * 100 : 0;
        const prevOpenRate = prevDelivered > 0 ? (prevOpened / prevDelivered) * 100 : 0;
        const openRateChange = currentOpenRate - prevOpenRate;
        
        const currentClickRate = totalOpened > 0 ? (totalClicked / totalOpened) * 100 : 0;
        const prevClicked = prevLogs.filter(l => l.deliveryStatus === 'clicked').length;
