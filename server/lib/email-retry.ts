...
export async function retryEmail(emailLogId: number): Promise<RetryResult> {
  ...
  try {
    ...
    const fromAddress = emailLog.emailType === 'invoice' 
    ...
    console.log(`[Email Retry] Successfully retried email ${emailLogId} on attempt ${newRetryCount}`);
    ...
    console.error(`[Email Retry] Failed to retry email ${emailLogId}:`, errorMessage);
...
export async function processFailedEmails(): Promise<{
  ...
  if (processed > 0) {
    console.log(`[Email Retry] Processed ${processed} emails: ${succeeded} succeeded, ${failed} failed`);
  }
