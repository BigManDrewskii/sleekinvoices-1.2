...
vi.mock('./storage', () => ({
  storagePut: vi.fn().mockResolvedValue({ url: 'https://example.com/export.json', key: 'exports/1/test.json' }),
}));
...
describe('Production Features', () => {
  ...
  describe('Invoice Duplication', () => {
    it('should create a new invoice with draft status', async () => {
      ...
      const lineItems = [
        { id: 1, invoiceId: 1, description: 'Service', quantity: '1', rate: '1000.00', amount: '1000.00', sortOrder: 0 },
      ];
      ...
      vi.mocked(db.createInvoice).mockResolvedValue({ id: 2, invoiceNumber: 'INV-002' } as any);
    ...
    it('should copy all line items to the new invoice', async () => {
      const lineItems = [
        { description: 'Item 1', quantity: '1', rate: '100.00', amount: '100.00' },
        { description: 'Item 2', quantity: '2', rate: '50.00', amount: '100.00' },
      ];
  ...
  describe('GDPR Data Export', () => {
    it('should gather all user data for export', async () => {
      ...
      vi.mocked(db.getClientsByUserId).mockResolvedValue([{ id: 1, name: 'Client 1' }] as any);
      vi.mocked(db.getInvoicesByUserId).mockResolvedValue([{ id: 1, invoiceNumber: 'INV-001' }] as any);
      vi.mocked(db.getProductsByUserId).mockResolvedValue([{ id: 1, name: 'Product 1' }] as any);
      vi.mocked(db.getExpensesByUserId).mockResolvedValue([{ id: 1, amount: '100.00' }] as any);
      vi.mocked(db.getInvoiceTemplatesByUserId).mockResolvedValue([{ id: 1, name: 'Template 1' }] as any);
      vi.mocked(db.getRecurringInvoicesByUserId).mockResolvedValue([]);
      vi.mocked(db.getEstimatesByUserId).mockResolvedValue([]);
      vi.mocked(db.getPaymentsByUserId).mockResolvedValue([{ id: 1, amount: '100.00' }] as any);
      vi.mocked(db.getEmailLogsByUserId).mockResolvedValue([{ id: 1, subject: 'Invoice' }] as any);
  ...
  describe('Audit Logging', () => {
    ...
    it('should retrieve audit logs with pagination', async () => {
      ...
      vi.mocked(db.getAuditLogs).mockResolvedValue({
        logs: [
          { id: 1, action: 'create', entityType: 'invoice', createdAt: new Date() },
          { id: 2, action: 'update', entityType: 'invoice', createdAt: new Date() },
        ],
        total: 10,
      } as any);
  ...
  describe('Email Delivery Tracking', () => {
    it('should have correct delivery status enum values', () => {
      const validStatuses = ['sent', 'delivered', 'opened', 'clicked', 'bounced', 'complained', 'failed'];
