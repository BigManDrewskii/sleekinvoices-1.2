...
import { encrypt, decrypt, encryptJSON, decryptJSON, generateEncryptionKey } from './lib/encryption';
...
describe('Phase 1 Foundation Tests', () => {
  ...
  afterAll(async () => {
    ...
    await db.delete(expenseCategories).where(eq(expenseCategories.userId, testUserId));
  ...
  describe('1.4 Invoice View Tracking', () => {
    ...
    it('1.4.1 should create invoice view records', async () => {
      ...
      // Create view record
      const [view] = await db.insert(invoiceViews).values({
        invoiceId: invoice.id,
    ...
    it('should track multiple views for same invoice', async () => {
      ...
      // Cleanup
      await db.delete(invoiceViews).where(eq(invoiceViews.invoiceId, invoice.id));
      await db.delete(invoices).where(eq(invoices.id, invoice.id));
    });
