...
async function testClientsAPI() {
  ...
  try {
    ...
    const [clients] = await connection.query('SELECT * FROM clients WHERE userId = 1 LIMIT 5');
    ...
    const [contacts] = await connection.query('SELECT clientId, COUNT(*) as count FROM clientContacts GROUP BY clientId LIMIT 5');
    ...
    if (allClients.length === 0) {
      ...
      const [allClientsAnyUser] = await connection.query('SELECT userId, COUNT(*) as count FROM clients GROUP BY userId');
      console.log('\nClients by userId:');
