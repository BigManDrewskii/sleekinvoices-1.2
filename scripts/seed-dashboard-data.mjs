  ...
  try {
    ...
    for (const month of months) {
      ...
      for (let i = 0; i < invoicesThisMonth; i++) {
        ...
        const subtotal = Math.floor(Math.random() * (range.max - range.min) + range.min);
        ...
        const daysToAdd = termsOptions[Math.floor(Math.random() * termsOptions.length)];
        ...
        if (daysSinceIssue > 60) {
          ...
          if (rand < 0.4) {
            status = 'paid';
            amountPaid = total;
            paidAt = new Date(issueDate.getTime() + Math.random() * 25 * 24 * 60 * 60 * 1000);
          } else if (rand < 0.7) {
            status = 'overdue';
          }
        } else {
          ...
          if (rand < 0.2) {
            status = 'paid';
            amountPaid = total;
            paidAt = new Date(issueDate.getTime() + Math.random() * 20 * 24 * 60 * 60 * 1000);
          } else if (rand < 0.9) {
            status = 'sent';
          } else if (rand < 0.95) {
            status = 'overdue';
      ...
      console.log(`   ✓ ${invoicesThisMonth} invoices | Revenue: $${monthRevenue.toFixed(2)} | Paid: $${monthPaid.toFixed(2)}`);
    }
    ...
    const [expenseCategories] = await connection.query('SELECT id, name FROM expenseCategories');
    ...
    for (const invoice of recentInvoices) {
      ...
      for (let i = 0; i < viewCount; i++) {
        const viewDate = getRandomDate(
          ...
          new Date(Math.min(now, new Date(invoice.createdAt).getTime() + 30 * 24 * 60 * 60 * 1000))
        );
    ...
    console.log(`   ✓ Added view tracking for ${recentInvoices.length} invoices`);
    ...
    console.log(`   • Payment rate: ${((paidRevenue / totalRevenue) * 100).toFixed(1)}%`);
    console.log(`   • Average invoice value: $${(totalRevenue / invoiceCount).toFixed(2)}`);
