...
const SAMPLE_CLIENTS = [
  { name: 'Acme Corporation', email: 'billing@acme.com', companyName: 'Acme Corp', address: '123 Business Ave, New York, NY 10001', phone: '+1 (555) 123-4567' },
  { name: 'TechStart Inc', email: 'accounts@techstart.io', companyName: 'TechStart Inc', address: '456 Innovation Blvd, San Francisco, CA 94105', phone: '+1 (555) 234-5678' },
  { name: 'Global Ventures', email: 'finance@globalventures.com', companyName: 'Global Ventures LLC', address: '789 Enterprise Way, Austin, TX 78701', phone: '+1 (555) 345-6789' },
  { name: 'Creative Agency', email: 'hello@creativeagency.co', companyName: 'Creative Agency Co', address: '321 Design District, Los Angeles, CA 90001', phone: '+1 (555) 456-7890' },
  { name: 'DataFlow Systems', email: 'ap@dataflow.systems', companyName: 'DataFlow Systems', address: '654 Tech Park, Seattle, WA 98101', phone: '+1 (555) 567-8901' },
  { name: 'Green Energy Co', email: 'invoices@greenenergy.co', companyName: 'Green Energy Corporation', address: '987 Sustainable Lane, Denver, CO 80201', phone: '+1 (555) 678-9012' },
  { name: 'Retail Plus', email: 'accounting@retailplus.com', companyName: 'Retail Plus Inc', address: '147 Commerce St, Chicago, IL 60601', phone: '+1 (555) 789-0123' },
  { name: 'HealthTech Solutions', email: 'billing@healthtech.io', companyName: 'HealthTech Solutions', address: '258 Medical Center Dr, Boston, MA 02101', phone: '+1 (555) 890-1234' },
];

const SAMPLE_PRODUCTS = [
  { name: 'Web Development', description: 'Full-stack web development services', rate: '150.00', unit: 'hour', category: 'Development' },
  { name: 'UI/UX Design', description: 'User interface and experience design', rate: '125.00', unit: 'hour', category: 'Design' },
  { name: 'Consulting', description: 'Technical consulting and advisory', rate: '200.00', unit: 'hour', category: 'Consulting' },
  { name: 'Project Management', description: 'Project planning and coordination', rate: '100.00', unit: 'hour', category: 'Management' },
  { name: 'Monthly Retainer', description: 'Monthly support and maintenance retainer', rate: '2500.00', unit: 'month', category: 'Retainer' },
  { name: 'Logo Design', description: 'Custom logo design package', rate: '500.00', unit: 'project', category: 'Design' },
  { name: 'SEO Audit', description: 'Comprehensive SEO analysis and recommendations', rate: '750.00', unit: 'project', category: 'Marketing' },
  { name: 'API Integration', description: 'Third-party API integration services', rate: '175.00', unit: 'hour', category: 'Development' },
];
...
const INVOICE_TEMPLATES = [
  { name: 'Sleek Dark', templateType: 'sleek', primaryColor: '#5f6fff', secondaryColor: '#252f33', accentColor: '#10b981', isDefault: true },
  { name: 'Modern Blue', templateType: 'modern', primaryColor: '#3b82f6', secondaryColor: '#1e293b', accentColor: '#22c55e', isDefault: false },
  { name: 'Classic Professional', templateType: 'classic', primaryColor: '#1f2937', secondaryColor: '#f3f4f6', accentColor: '#2563eb', isDefault: false },
  { name: 'Minimal Clean', templateType: 'minimal', primaryColor: '#000000', secondaryColor: '#ffffff', accentColor: '#6366f1', isDefault: false },
];
  ...
  try {
    ...
    for (const client of SAMPLE_CLIENTS) {
      const [result] = await connection.execute(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `, [userId, client.name, client.email, client.companyName, client.address, client.phone]);
    ...
    for (const product of SAMPLE_PRODUCTS) {
      await connection.execute(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, true, true)
      `, [userId, product.name, product.description, product.rate, product.unit, product.category]);
    }
    ...
    for (const template of INVOICE_TEMPLATES) {
      ...
      `, [userId, template.name, template.templateType, template.primaryColor, template.secondaryColor, template.accentColor, template.isDefault]);
    }
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const issueDate = randomDate(new Date(now.getFullYear(), now.getMonth() - 3, 1), now);
      const dueDate = new Date(issueDate);
      dueDate.setDate(dueDate.getDate() + 30);
      
      const subtotal = (Math.random() * 5000 + 500).toFixed(2);
      const taxRate = '10.00';
      const taxAmount = (parseFloat(subtotal) * 0.10).toFixed(2);
      ...
      `, [userId, clientId, invoiceNumber, status, subtotal, taxRate, taxAmount, total, amountPaid, formatDate(issueDate), formatDate(dueDate)]);
      ...
      for (let j = 0; j < lineItemCount; j++) {
        ...
        await connection.execute(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `, [invoiceId, `${product.name} - ${product.description}`, quantity, rate, amount, j]);
      }
    ...
    const vendors = ['Adobe', 'AWS', 'Google', 'Microsoft', 'Zoom', 'Slack', 'Figma', 'GitHub'];
    let expenseCount = 0;
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const date = randomDate(new Date(now.getFullYear(), now.getMonth() - 3, 1), now);
      
      await connection.execute(`
        INSERT INTO expenses (userId, categoryId, amount, currency, date, vendor, description, isTaxDeductible)
        VALUES (?, ?, ?, 'USD', ?, ?, ?, true)
      `, [userId, categoryId, amount, formatDate(date), vendor, `${vendor} subscription/service`]);
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const issueDate = randomDate(new Date(now.getFullYear(), now.getMonth() - 1, 1), now);
      ...
      `, [userId, clientId, estimateNumber, status, subtotal, taxRate, taxAmount, total, formatDate(issueDate), formatDate(validUntil)]);
    }
