...
async function fixTemplatesTable() {
  ...
  try {
    ...
    const [columns] = await connection.query('SHOW COLUMNS FROM invoiceTemplates');
    ...
    if (missingColumns.length > 0) {
      ...
      for (const column of missingColumns) {
        try {
          await connection.query(`ALTER TABLE invoiceTemplates ADD COLUMN ${column} ${expectedColumns[column]}`);
