...
async function verifyClients() {
  ...
  try {
    ...
    console.log(`   Clients with contacts: ${withContacts.length}/${clients.length} (${Math.round(withContacts.length/clients.length*100)}%)`);
    console.log(`   Clients with notes: ${withNotes.length}/${clients.length} (${Math.round(withNotes.length/clients.length*100)}%)`);
    console.log(`   Complete profiles (all fields + contacts): ${complete.length}/${clients.length} (${Math.round(complete.length/clients.length*100)}%)`);
    ...
    if (sampleContacts.length > 0) {
      ...
      sampleContacts.forEach((contact) => {
        console.log(`   • ${contact.firstName} ${contact.lastName} - ${contact.role}${contact.isPrimary ? ' (Primary)' : ''}`);
