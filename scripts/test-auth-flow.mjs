...
async function testAuthFlow() {
  ...
  try {
    ...
    const [users] = await connection.query('SELECT id, email, name FROM users WHERE email = ?', ['dev@sleekinvoices.local']);
    ...
    if (users.length === 0) {
      ...
      const [result] = await connection.query(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `, ['dev@sleekinvoices.local', 'Dev User', 'dev-user-bypass', 'active', new Date('2027-12-31')]);

      console.log(`✅ Created dev user with ID: ${result.insertId}`);
    } else {
      console.log(`✅ Dev user found: ID=${users[0].id}, Email=${users[0].email}`);
    }

    // Get the dev user ID
    const [devUser] = await connection.query('SELECT id FROM users WHERE email = ?', ['dev@sleekinvoices.local']);
    const userId = devUser[0].id;

    // Check clients for this user
    const [clients] = await connection.query('SELECT id, name, email FROM clients WHERE userId = ?', [userId]);
    ...
    const [otherClients] = await connection.query('SELECT userId, COUNT(*) as count FROM clients GROUP BY userId');
    console.log(`\n📋 Clients by userId:`);
    ...
    if (otherClients.length > 0 && otherClients.some((row) => row.userId !== userId)) {
      console.log(`\n⚠️  Found clients with different userId. Updating to match dev user...`);

      await connection.query('UPDATE clients SET userId = ? WHERE userId != ?', [userId, userId]);
      console.log('✅ Updated all clients to use dev user ID');

      // Verify the update
      const [updated] = await connection.query('SELECT COUNT(*) as count FROM clients WHERE userId = ?', [userId]);
      console.log(`   Now have ${updated[0].count} clients for userId=${userId}`);
    }
