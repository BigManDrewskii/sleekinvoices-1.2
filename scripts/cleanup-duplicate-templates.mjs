...
const allTemplates = await db
  ...
  .orderBy(schema.invoiceTemplates.userId, schema.invoiceTemplates.name, schema.invoiceTemplates.createdAt);
...
for (const [userId, templates] of Object.entries(templatesByUser)) {
  ...
  for (const [name, instances] of Object.entries(templates)) {
    if (instances.length > 1) {
      console.log(`  📋 "${name}": ${instances.length} instances (keeping oldest, removing ${instances.length - 1})`);
...
console.log(`  Templates after cleanup: ${allTemplates.length - totalDuplicates}`);
