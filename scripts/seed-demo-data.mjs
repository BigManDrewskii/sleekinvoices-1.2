...
const demoClients = [
  { name: 'Acme Corporation', email: 'billing@acme.com', phone: '+1 (555) 123-4567', address: '123 Business Ave, Suite 100, New York, NY 10001', companyName: 'Acme Corporation' },
  { name: 'TechStart Inc', email: 'finance@techstart.io', phone: '+1 (555) 987-6543', address: '456 Innovation Drive, San Francisco, CA 94105', companyName: 'TechStart Inc' },
  { name: 'Global Services Ltd', email: 'accounts@globalservices.co.uk', phone: '+44 20 7123 4567', address: '78 Queen Street, London EC1N 8AA, UK', companyName: 'Global Services Ltd' },
  { name: 'Smith & Associates', email: 'admin@smithlaw.com', phone: '+1 (555) 456-7890', address: '101 Legal Plaza, Chicago, IL 60601', companyName: 'Smith & Associates' },
  { name: 'Creative Studios', email: 'hello@creativestudios.com', phone: '+1 (555) 321-0987', address: '222 Art District, Los Angeles, CA 90012', companyName: 'Creative Studios' },
  { name: 'Mountain View Properties', email: 'rentals@mtnview.com', phone: '+1 (555) 789-0123', address: '333 Summit Road, Denver, CO 80202', companyName: 'Mountain View Properties' },
  { name: 'Pacific Northwest Coffee', email: 'orders@pnwcoffee.com', phone: '+1 (555) 234-5678', address: '444 Roasters Way, Seattle, WA 98101', companyName: 'Pacific Northwest Coffee' },
  { name: 'Digital Dynamics Agency', email: 'client-services@digitaldynamics.com', phone: '+1 (555) 876-5432', address: '555 Tech Boulevard, Austin, TX 78701', companyName: 'Digital Dynamics Agency' },
  { name: 'Green Leaf Organics', email: 'sales@greenleaforganics.com', phone: '+1 (555) 345-6789', address: '666 Farm Road, Portland, OR 97201', companyName: 'Green Leaf Organics' },
  { name: 'Metro Construction Co', email: 'billing@metroconstruction.com', phone: '+1 (555) 210-9876', address: '777 Builders Lane, Phoenix, AZ 85001', companyName: 'Metro Construction Co' },
  { name: 'Sunrise Dental Care', email: 'office@sunrisedental.com', phone: '+1 (555) 654-3210', address: '888 Medical Center Dr, Miami, FL 33101', companyName: 'Sunrise Dental Care' },
  { name: 'Elite Fitness Center', email: 'membership@elitefitness.com', phone: '+1 (555) 098-7654', address: '999 Gym Street, Boston, MA 02101', companyName: 'Elite Fitness Center' },
];
...
const paymentTerms = ['Net 15', 'Net 30', 'Net 60', 'Due on receipt', 'Due in 7 days'];
  ...
  try {
    ...
    for (const client of demoClients) {
      try {
        const [result] = await connection.query(
          ...
          [userId, client.name, client.email, client.phone, client.address, client.companyName]
        );
    ...
    for (const client of clients) {
      ...
      for (let i = 0; i < numInvoices; i++) {
        ...
        else if (terms === 'Due in 7 days') dueDate.setDate(dueDate.getDate() + 7);
        ...
        if (status === 'paid') {
          amountPaid = total;
          paidAt = new Date(issueDate.getTime() + Math.random() * 30 * 24 * 60 * 60 * 1000);
        ...
        for (const item of lineItems) {
          await connection.query(
            ...
            [invoiceId, item.description, item.quantity, item.rate.toFixed(2), item.amount.toFixed(2)]
          );
        }

        // Add payments for paid/partially paid invoices
        if (amountPaid > 0) {
          const paymentDate = paidAt || new Date(issueDate.getTime() + Math.random() * (dueDate.getTime() - issueDate.getTime()));
          await connection.query(
            `INSERT INTO payments (
              invoiceId, userId, amount, paymentMethod, paymentDate, status, createdAt, updatedAt
            ) VALUES (?, ?, ?, ?, ?, 'completed', NOW(), NOW())`,
            [invoiceId, userId, amountPaid.toFixed(2), 'bank_transfer', paymentDate]
          );
        }

        invoiceCount++;
        console.log(`   ✓ ${invoiceNumber}: ${status.toUpperCase()} - $${total.toFixed(2)} (${clients.length > 5 ? demoClients.find(c => c.name.includes(client.id.toString()))?.name || 'Client' : 'Client'})`);
      }
    }

    // Create some recurring invoices
    console.log('\n🔄 Creating recurring invoices...');
    const [activeClients] = await connection.query('SELECT id FROM clients LIMIT 5');
    const recurringIntervals = ['weekly', 'monthly', 'yearly'];

    for (const client of activeClients) {
      const interval = recurringIntervals[Math.floor(Math.random() * recurringIntervals.length)];
      ...
      console.log(`   ✓ Created recurring invoice: ${template.name} (${interval})`);
    }
    ...
    console.log('\n💸 Creating expenses...');
    const [expenseCategories] = await connection.query('SELECT id, name FROM expenseCategories LIMIT 4');
    const expenses = [
      { amount: 99.00, description: 'Adobe Creative Cloud', vendor: 'Adobe Inc' },
      { amount: 49.00, description: 'GitHub Pro Account', vendor: 'GitHub' },
      { amount: 299.00, description: 'Google Workspace', vendor: 'Google' },
      { amount: 150.00, description: 'Office supplies', vendor: 'Staples' },
      { amount: 2500.00, description: 'Conference attendance', vendor: 'TechConf 2026' },
      { amount: 89.00, description: 'Internet & Phone', vendor: 'Comcast Business' },
    ];
    ...
    for (let i = 0; i < expenses.length; i++) {
      ...
      console.log(`   ✓ Created expense: ${expense.description} - $${expense.amount.toFixed(2)}`);
    }
